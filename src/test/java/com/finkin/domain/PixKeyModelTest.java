...
class PixKeyModelTest {
    ...
    void shouldCreateCpfKey() {
        var key = PixKeyModel.create(ACCOUNT_ID, PixKeyTypeEnum.CPF, "529.982.247-25");
        assertThat(key.getKeyValue()).isEqualTo("52998224725");
        assertThat(key.getKeyType()).isEqualTo(PixKeyTypeEnum.CPF);
    }

    @Test
    void shouldRejectInvalidCpfKey() {
        assertThatThrownBy(() -> PixKeyModel.create(ACCOUNT_ID, PixKeyTypeEnum.CPF, "111.111.111-11"))
            .isInstanceOf(InvalidPixKeyException.class);
    }

    @Test
    void shouldCreateEmailKey() {
        var key = PixKeyModel.create(ACCOUNT_ID, PixKeyTypeEnum.EMAIL, "usuario@finkin.dev");
    ...
    void shouldRejectInvalidEmailKey(String email) {
        assertThatThrownBy(() -> PixKeyModel.create(ACCOUNT_ID, PixKeyTypeEnum.EMAIL, email))
            .isInstanceOf(InvalidPixKeyException.class);
    }

    @Test
    void shouldCreatePhoneKey() {
        var key = PixKeyModel.create(ACCOUNT_ID, PixKeyTypeEnum.PHONE, "+5511999999999");
    ...
    void shouldRejectInvalidPhoneKey(String phone) {
        assertThatThrownBy(() -> PixKeyModel.create(ACCOUNT_ID, PixKeyTypeEnum.PHONE, phone))
            .isInstanceOf(InvalidPixKeyException.class);
    }
    ...
    void shouldCreateRandomKey() {
        ...
        assertThat(key.getKeyType()).isEqualTo(PixKeyTypeEnum.RANDOM);
