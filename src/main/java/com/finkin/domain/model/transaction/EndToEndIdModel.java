...
public class EndToEndIdModel {
    ...
    public EndToEndIdModel(String value) {
        if (value == null || value.length() != BankConstants.END_TO_END_ID_LENGTH) {
            throw new IllegalArgumentException("EndToEndIdModel inválido: deve ter 32 chars");
        }
        this.value = value;
    }

    /** Gera um novo EndToEndIdModel com o timestamp atual de Brasília. */
    public static EndToEndIdModel generate() {
        ZonedDateTime now = ZonedDateTime.now(ZoneId.of(BankConstants.TIMEZONE_BR));
        return generate(now);
    }

    /** Overload para injeção de tempo em testes. */
    public static EndToEndIdModel generate(ZonedDateTime now) {
        ...
        String raw = BankConstants.END_TO_END_PREFIX
            ...
            + "0"
            + suffix;
        assert raw.length() == BankConstants.END_TO_END_ID_LENGTH : "EndToEndIdModel deve ter 32 chars, tem " + raw.length();
        return new EndToEndIdModel(raw);
    }
